...
SamplePieceExp <- function(X,Z,H_norm,H_prime,Pcum){
  ...
  print(paste("a_coef: ",a_coef))
  print(paste("b_coef: ",b_coef))
  print(paste("DP: ",DP))
  print(paste("Zbin: ",Z[i_bin]))
...
UpdateAccept <- function(x_star, X ,Z , H, H_prime, H_norm, P_cum, x_accept, length_accept ,eval_h ,eval_h_prime){
  ...
  z <- function(i) {
    if (abs(H_prime[i]-H_prime[i+1]) < .Machine$double.eps*H_prime[i]){
