...
$$
A_i = \int^{z_{i+1}}_{z_i} e^{h(z_i) + (x-z_i) h'(x_i)} dx = e^h(z_i) \frac{e^{-z_i h'(x_i)}}{h'(x_i)} (e^{h'(x_i) z_i} - e^{h'(x_i) z_{i+1}})
$$
...
CalcProbBin <- function(X,Z,H,H_prime){
  ...
  #Output:
  # Pcum: numeric array with the cumulative probability of each of each segment 
  # s: numeric array with the normalized elevations of exponential distribution at locations X
